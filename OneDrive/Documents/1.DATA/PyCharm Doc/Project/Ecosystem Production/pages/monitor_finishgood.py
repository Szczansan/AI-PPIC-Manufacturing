...
def load_integrated_data(date_str):
    """Mengambil data dari forecast_monthly, MASTER, dan Saldo Berjalan dari View v_stock_balance."""
    try:
        # 1. LOAD FORECAST data (MENJADI BASE TABLE)
        # UPDATE: Menggunakan tabel baru 'forecast_monthly'
        # Kolom yang diambil: part_no, part_name (dari trigger), forecast_month
        res_fc = supabase.table("forecast_monthly").select("part_no, part_name, forecast_month").execute()
        df_fc = pd.DataFrame(res_fc.data)

        if df_fc.empty:
            return pd.DataFrame()

        # Normalisasi nama kolom dari tabel forecast_monthly ke standar aplikasi
        # forecast_month -> FORCAST_DATE_REF (supaya logic di bawah tetap jalan)
        # part_no -> PART_NO
        # part_name -> PART_NAME
        
        # Kita map secara eksplisit karena struktur tabel baru sudah jelas
        rename_map = {
            'part_no': 'PART_NO',
            'part_name': 'PART_NAME',
            'forecast_month': 'FORCAST_DATE_REF'
        }
        
        # Lakukan rename jika kolom ada
        df_fc = df_fc.rename(columns=rename_map)

        # Validasi apakah kolom penting ada setelah rename
        if 'PART_NO' in df_fc.columns:
            # Base table dari forcast (PART_NO harus unik untuk join awal, atau handle duplikat)
            # Mengambil data unik berdasarkan PART_NO untuk list part dasar
            df_base = df_fc[['PART_NO', 'PART_NAME', 'FORCAST_DATE_REF']].drop_duplicates(subset=['PART_NO'], keep='first')
        else:
            raise ValueError(f"Kolom part_no tidak ditemukan di tabel 'forecast_monthly'.")
        ...
        # Rename kolom master agar sesuai key join
        if not df_master.empty:
            df_master = df_master.rename(columns={'part_no': 'PART_NO'})
        else:
            df_master = pd.DataFrame(columns=['PART_NO', 'SPQ'])
        ...
        if not df_history.empty:
            ...
            df_final = pd.merge(df_final, df_stock_latest, on="PART_NO", how="left")
        ...
        if 'FORCAST DATE' in df_final.columns:
            # Karena forecast_month dari DB text, kita convert dulu ke datetime agar aman
        ...
        st.error(f"Gagal mengambil atau menggabungkan data dari Supabase. Pastikan View 'v_stock_balance' dan tabel 'forecast_monthly', 'MASTER' ada. Detail Error: {e}")
        st.stop()
        return pd.DataFrame()
