...
try:
    # Kita tambah 'machine_id' di query
    response = supabase.table("MASTER").select("TYPE_MATERIAL, GRADE_MATERIAL, COLOR_MATERIAL, machine_id").execute()
...
# Helper list
if not df_ref.empty:
    list_type = sorted(df_ref["TYPE_MATERIAL"].dropna().unique().tolist())
    list_grade = sorted(df_ref["GRADE_MATERIAL"].dropna().unique().tolist())
    list_color = sorted(df_ref["COLOR_MATERIAL"].dropna().unique().tolist())
    # Tambahan baru untuk Line Production
    list_line = sorted(df_ref["machine_id"].dropna().unique().tolist())
else:
    list_type, list_grade, list_color, list_line = [], [], [], []
...
with st.container():
    with st.form("out_form", clear_on_submit=False):
        ...
        with c7:
            # Menggunakan list_line dari database
            in_line = st.selectbox("Line Production", list_line) if list_line else st.selectbox("Line Production", ["No Data"])
        with c8:
